...
import { BlogPostItem } from '@/data/blogs-data'
...
export function BlogsClient({ initialPosts }: BlogsClientProps) {
  ...
  const filteredBlogs =
    activeCategory === 'ALL'
      ...
      : initialPosts.filter(
          (blog) =>
            blog.category?.toUpperCase() === activeCategory ||
            (activeCategory === 'FRACTIONAL GUIDE' && !blog.category)
        )
          ...
          {filteredBlogs.map((blog) => (
            <article key={blog.id} className="blog-card">
              <div className="blog-card-img-wrap" style={{ height: '220px', position: 'relative' }}>
                ...
                >
                  {blog.category || 'FRACTIONAL GUIDE'}
                </span>
              </div>
